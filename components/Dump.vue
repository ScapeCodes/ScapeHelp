    ...
    <b-message type="is-primary" v-if="invalid">
      <div class="content">
        <p>
          ...
          <br/>
          You can then click on the provided link to view the dump, or you can give the link to <a class="has-text-white" href="https://discord.gg/D5FnJ7C">CraftationGaming</a> support when getting help.
        </p>
        <p>
    ...
    <div v-if="loaded">
        ...
        <div class="message-body">
          <nav class="level">
            <div class="level-item has-text-centered">
              <div>
                <p class="heading">Plugin Name</p>
                <p class="title is-6">{{ pluginName }}</p>
              </div>
            </div>
            <div class="level-item has-text-centered">
              <div>
      ...
      <b-notification type="is-dark" :closable="false">
        <p>
          ...
          <b>{{ senderName }}</b>
          at {{ new Date(timestamp).toLocaleString() }} using <a :href="originalUrl" target="_blank">Craftation Paste</a>.
        </p>
      </b-notification>
    </div>
  </div>
    ...
    async loadData() {
      ...
      try {
        const { data } = await axios.get(`https://paste.craftationgaming.com/raw/${this.pasteId}`)
            ...
            const dump = data
            this.pluginName = dump["plugin-data"].name
